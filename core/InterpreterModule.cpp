...
void InterpreterModule::readConfig() {
  ...
  if (memory_->core_type_ == CORE_ROBOT) {
    auto& config = core_->rconfig() = RobotConfig();
    tfm::printf("Reading config file: %s\n", util::cfgpath(util::RobotConfig));
    config.load(util::cfgpath(util::RobotConfig));
    ...
    if(config.game_controller_ip != "")
      CommInfo::GAME_CONTROLLER_IP = config.game_controller_ip;
    if(config.team_udp != 0)
      CommInfo::TEAM_UDP_PORT = config.team_udp;
    robot_state_->role_ = robot_state_->WO_SELF = robot_state_->global_index_ = config.self;
