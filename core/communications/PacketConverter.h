...
class PacketConverter {
  public:
    static TeamPacket convert(SPLStandardMessage& message) {
      ...
      std_data outputdata = compressor.decompress<std_data>(message.data, message.numOfDataBytes, message.valid);
      ...
      output.bvrData.ballBid = outputdata.ballBid; output.bvrData.ballMissed = outputdata.ballMissed;
      ...
      output.locData.setBallPos(relBalls.relativeToGlobal(output.locData.robotPos(), output.locData.orientation()));
...
static SPLStandardMessage convert(TeamPacket message) {
  ...
  Point2D relBalls = balls.globalToRelative(message.locData.robotPos(), message.locData.orientation());
  if (message.bvrData.ballSeen()) {
    output.ball[0] = relBalls.x;
    output.ball[1] = relBalls.y;
    output.ballAge = message.bvrData.ballMissed*30;
  } else {
    output.ballAge = -1;
  }
  //stop filling in SPLStandardMessage fields
  ...
  outputdata.ballMissed = message.bvrData.ballMissed;
