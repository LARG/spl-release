...
{
  // We need to set a minimum size so that the interface matches that of SPLCompressedMessage,
  // but we leave it zero because any SPLStandardMessage that is sent will be, by definition,
  // of at least minimum size according to the standard.
  static constexpr std::size_t MinimumSize = 0;

  char header[4];        // "SPL "
  uint8_t version;       // has to be set to SPL_STANDARD_MESSAGE_STRUCT_VERSION
  int8_t robotNumber;      // [MANDATORY FIELD] 1-5 in drop-in games, 1-6 in normal games
  int8_t teamNumber;        // [MANDATORY FIELD] the number of the team (as provided by the organizers)
  ...
  inline std::size_t send_size() const { return offsetof(struct SPLStandardMessage, data) + numOfDataBytes; }
  bool valid = false;
