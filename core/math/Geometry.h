...
class Point2D
{
 ...
 public:
  ...
  Point2D rotate          ( Point2D origin, AngRad ang );
...
public:
  ...
  bool isParallelTo(Line2D other) const;
...
class LineSegment : public Line2D {
  public:
    LineSegment();
    LineSegment(int x1, int y1, int x2, int y2);
    LineSegment(Point2D start, Point2D end);
    float length() const;
    Point2D getPointOnSegmentClosestTo(Point2D p) const;
    Point2D getNearestCircleIntersection(Point2D center, float radius, Point2D near);
    float getDistanceTo(Point2D p) const;
    float getDistanceTo(Point2D p1, Point2D p2) const;
    float getSignedAngleToLine(Line2D line) const;
    Point2D getSegmentIntersection(Line2D line) const;
    bool intersects(const LineSegment& other) const;
    Point2D start, end;
    LineSegment relativeToGlobal(Point2D origin, AngRad ang);
    LineSegment globalToRelative(Point2D origin, AngRad ang);
    LineSegment rotate(Point2D origin, AngRad ang);
    LineSegment rotate(AngRad ang);
    LineSegment getVisiblePortion(Point2D origin, AngRad orientation, AngRad fov);
    LineSegment crop(LineSegment bounds);
