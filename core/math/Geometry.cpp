...
/*! String coversion for Python
  \return string representation of point */
const char* Point2D::__str__() const
{
...
bool Point2D::isBetweenTwoPoints( const Point2D &p1, 
  ...
  if(p1.x < p2.x) {
    if(x < p1.x - EPSILON || x > p2.x + EPSILON) return false;
  } else {
    if(x < p2.x - EPSILON || x > p1.x + EPSILON) return false;
  }
  if(p1.y < p2.y) {
    if(y < p1.y - EPSILON || y > p2.y + EPSILON) return false;
  } else {
    if(y < p2.y - EPSILON || y > p1.y + EPSILON) return false;
  }
  return true;
}
...
Point2D Point2D::rotate(Point2D origin, AngRad ang)
{
  *this -= origin;
  rotate(ang);
  *this += origin;
...
AngRad Line2D::getAngle() const {
  float angle = atan2f(-m_b, m_a);
  return normalizeAngleNinety(angle);
}
...
bool LineSegment::intersects(const LineSegment& other) const {
  auto p = getSegmentIntersection(other);
  if(p.isBetweenTwoPoints(start, end) && p.isBetweenTwoPoints(other.start, other.end))
    return true;
  return false;
}
...
std::ostream& operator<<(std::ostream& os, const LineSegment& ls) {
  ...
  os.precision(2);
  os << "" << ls.start << " --> " << ls.end << "[" << ls.getDirection() * RAD_T_DEG << "]";
  return os;
}
