...
void CKF::update(const SensorValues &sensorValues){
   ...
   float Ax = sensorValues.sensors[RSSensors::InertialSensor_AccX];
   float Ay = sensorValues.sensors[RSSensors::InertialSensor_AccY];
   float Az = sensorValues.sensors[RSSensors::InertialSensor_AccZ];

   obs(0) = atan(Ay/Az); // Roll
   obs(1) = atan(-Ax/Az)*cos(obs(0)); // Pitch
   obs(2) = sensorValues.sensors[RSSensors::InertialSensor_GyrX]; // Roll velocity
   obs(3) = sensorValues.sensors[RSSensors::InertialSensor_GyrY]; // Pitch velocity
