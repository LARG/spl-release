                         ...
                         filZMPL(0.0)
{
   ...
   isGyroXCalibrated = false;
   isGyroYCalibrated = false;
   isGyroZCalibrated = false;
   isLeftLegHot = false;
   isRightLegHot = false;
...
bool BodyModel::isFootOnGround(const SensorValues &sensors) {
   float leftFrontL = sensors.sensors[RSSensors::LFoot_FSR_FrontLeft];
   float leftFrontR = sensors.sensors[RSSensors::LFoot_FSR_FrontRight];
   float rightFrontL = sensors.sensors[RSSensors::RFoot_FSR_FrontLeft];
   float rightFrontR = sensors.sensors[RSSensors::RFoot_FSR_FrontRight];
   float leftRearL  = sensors.sensors[RSSensors::LFoot_FSR_RearLeft];
   float leftRearR  = sensors.sensors[RSSensors::LFoot_FSR_RearRight];
   float rightRearL = sensors.sensors[RSSensors::RFoot_FSR_RearLeft];
   float rightRearR = sensors.sensors[RSSensors::RFoot_FSR_RearRight];
   if (isLeftPhase) {
...
bool BodyModel::isOnFrontOfFoot(const SensorValues &sensors) {
   float leftFront = sensors.sensors[RSSensors::LFoot_FSR_FrontLeft] +
                     sensors.sensors[RSSensors::LFoot_FSR_FrontRight];
   float rightFront = sensors.sensors[RSSensors::RFoot_FSR_FrontLeft] +
                      sensors.sensors[RSSensors::RFoot_FSR_FrontRight];
   float leftRear  = sensors.sensors[RSSensors::LFoot_FSR_RearLeft] +
                     sensors.sensors[RSSensors::LFoot_FSR_RearRight];
   float rightRear = sensors.sensors[RSSensors::RFoot_FSR_RearLeft] +
                     sensors.sensors[RSSensors::RFoot_FSR_RearRight];
   if (isLeftPhase) {
...
float BodyModel::getFootZMP(bool isLeft, const SensorValues &sensors) {
   ...
   if (isLeft) {
      totalPressure += sensors.sensors[RSSensors::LFoot_FSR_FrontLeft]
                         + sensors.sensors[RSSensors::LFoot_FSR_FrontRight]
                         + sensors.sensors[RSSensors::LFoot_FSR_RearLeft]
                         + sensors.sensors[RSSensors::LFoot_FSR_RearRight];
   } else {
      totalPressure += sensors.sensors[RSSensors::RFoot_FSR_FrontLeft]
                         + sensors.sensors[RSSensors::RFoot_FSR_FrontRight]
                         + sensors.sensors[RSSensors::RFoot_FSR_RearLeft]
                         + sensors.sensors[RSSensors::RFoot_FSR_RearRight];
   }
   if (isLeft) {
      ZMPF =  
            (75.0f) * sensors.sensors[RSSensors::LFoot_FSR_FrontLeft]
          + (-45.0f) * sensors.sensors[RSSensors::LFoot_FSR_RearLeft]
          + (75.0f) * sensors.sensors[RSSensors::LFoot_FSR_FrontRight]
          + (-45.0f) * sensors.sensors[RSSensors::LFoot_FSR_RearRight];
   } else {   
      ZMPF = 
           (75.0f) * sensors.sensors[RSSensors::RFoot_FSR_FrontRight]
         + (-45.0f) * sensors.sensors[RSSensors::RFoot_FSR_RearRight]
         + (75.0f) * sensors.sensors[RSSensors::RFoot_FSR_FrontLeft]
         + (-45.0f) * sensors.sensors[RSSensors::RFoot_FSR_RearLeft];
   }
...
float BodyModel::getHorizontalFootZMP(bool isLeft, const SensorValues &sensors) {
   ...
   if (isLeft) {
      ZMPF =  
            (20.0f) * sensors.sensors[RSSensors::LFoot_FSR_FrontLeft]
          + (20.0f) * sensors.sensors[RSSensors::LFoot_FSR_RearLeft]
          + (-20.0f) * sensors.sensors[RSSensors::LFoot_FSR_FrontRight]
          + (-20.0f) * sensors.sensors[RSSensors::LFoot_FSR_RearRight];
   } else {   
      ZMPF = 
           (-20.0f) * sensors.sensors[RSSensors::RFoot_FSR_FrontRight]
         + (-20.0f) * sensors.sensors[RSSensors::RFoot_FSR_RearRight]
         + (20.0f) * sensors.sensors[RSSensors::RFoot_FSR_FrontLeft]
         + (20.0f) * sensors.sensors[RSSensors::RFoot_FSR_RearLeft];
   }
   if (totalPressure == 0) return  0;
   ZMPF /= totalPressure;
   return ZMPF;
