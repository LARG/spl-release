...
BlobDetector::BlobDetector(DETECTOR_DECLARE_ARGS, ColorSegmenter& segmenter) :
  ...
  //Blob horizontalBlob[Color::NUM_Colors][MAX_LINE_BLOBS];
  for(int i = 0; i < Color::NUM_Colors; i++)
    horizontalBlob.push_back(std::vector<Blob>());

  //Blob verticalBlob[Color::NUM_Colors][MAX_LINE_BLOBS];
  for(int i = 0; i < Color::NUM_Colors; i++)
    ...
    for (int y = 0; y < iparams_.height - vstep; y += vstep) {
      for (uint16_t i = 0; i < horizontalPointCount[color][y]; i++) {
        ...
        if (y > 0 && horizontalPoint[color][y][i].lbIndex != (uint16_t)-1) {
          ...
          tlog(30, "****MAX_LINE_BLOBS exceeded while forming blobs. Please increase in Vision.h");
        }
...
void BlobDetector::calculateVerticalBlobData (unsigned char color) {
  ...
  for (uint16_t i = 0; i < verticalBlob[color].size(); i++) {
    ...
    tlog(45,  "Blob %i:- Starts at (%i, %i), Ends at (%i, %i), Num Line Points: %i, Calculate Values = (%f, %f, %f), width = (%i, %i)", i, x0, y0, x3, y3, lbi->lpCount, lbi->diffStart, lbi->diffEnd, lbi->doubleDiff, lbi->widthStart, lbi->widthEnd);
  }
  //std::cout << "\n";
}
...
void BlobDetector::calculateHorizontalBlobData(unsigned char color) {
  ...
  for (uint16_t i = 0; i < horizontalBlob[color].size(); i++) {
    ...
    tlog(45,  "Blob %i: (%i, %i) - %f, %f, %f, %i, %i", i, x0, y0, lbi->diffStart, lbi->diffEnd, lbi->doubleDiff, lbi->widthStart, lbi->widthEnd);
  }
}
        ...
        if (lpi->xi < lpj->xf && lpi->xf > lpj->xi) {
          ...
          if (y > 0 && lpi->lbIndex != (uint16_t)-1) {
            ...
            if(bl->lpCount >= MAX_BLOB_VISIONPOINTS) {
                tlog(30, "****Max line points exceeded while forming %s blobs. Please increase in Blob.h", getName(color));
            }
            else {
                bl->lpIndex[bl->lpCount++] = (y + vstep) << 16 | j;
            ...
            tlog(30, "Created %s blob", getName(color));
            // bound check
          } else {
            tlog(30, "****MAX_LINE_BLOBS exceeded while forming %s blobs. Please increase in Vision.h", getName(color));
          }
        }
      }
    }
...
void BlobDetector::calculateBlobData(BlobCollection& blobs, Color color) {
  ...
  for (unsigned int i = 0; i < blobs.size(); i++) {
    ...
    tlog(30, "%s blob %i calculated: (xi:xf,yi:yf) = (%i:%i,%i:%i)", getName(color), i, bl->xi, bl->xf, bl->yi, bl->yf);
    bl->correctPixelRatio /= 0.5f * bl->dx * bl->dy;
    tlog(20,   "correctpixel[%i] = %f",i,bl->correctPixelRatio);
  }
}
...
uint16_t BlobDetector::mergeBlobs(BlobCollection& blobs, uint16_t *mergeIndex, int mergeCount) {
  ...
  for (uint16_t i = 0; i < blobs.size(); i++) {
    ...
    for (uint16_t j = i + 1; j < blobs.size(); j++) {
      Blob *blj = &blobs[j];
      //tlog(39, "comparing (%i,%i),(%i,%i) with (%i,%i),(%i,%i)", bli->xi, bli->xf, bli->yi, bli->yf, blj->xi, blj->xf, blj->yi, blj->yf);
      ...
      if ((overlapX && closeY) ||
          (closeX && overlapY) ||
          (overlapX && overlapY)) {
            //tlog(39,"blob %i overlaps with %i: closeX: %i closeY: %i overlapX: %i overlapY: %i", i, j, closeX, closeY, overlapX, overlapY);
    ...
    // FYI: This loop is incomplete.  After a blob merge, you have
