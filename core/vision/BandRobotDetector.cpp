...
BandRobotDetector::BandRobotDetector(DETECTOR_DECLARE_ARGS, ColorSegmenter*& segmenter, BlobDetector*& blob_detector) :
...
float BandRobotDetector::estimateBandDistanceByGreenBelow(Blob * band) {
  ...
  for (int y = yStart; y < iparams_.height; y += vstep) {
    ...
    //tlog(39, "Checking green below at y = %i from xi=%i to xf=%i, Green counter: %i, Other Counter: %i", y, xStart, xEnd, greenCounter, otherCounter);
...
void BandRobotDetector::sanitizeBandCandidate(Blob * band, Color color) {
  tlog(39, "sanitizing blob for %s band check, (x,y) = (%i,%i), avgWidth = %2.2f\n", getName(color), band->avgX, band->avgY, band->avgWidth);
  int hstep, vstep;
  color_segmenter_->getStepSize(hstep, vstep);
  ...
  int y;
  tlog(39, "scanning from x = %i to %i, y = %i to max(%i, %i)", xStart, xEnd, yStart, 0, (int)(yStart - 4 * band->avgWidth));
  ...
  tlog(39,  "Robot: Blob at (%i, %i) white at top found at %i, other pixels till then %i, width pixels = %i", band->avgX, band->avgY, y, otherCounter, (xEnd - xStart));
  if (otherCounter > float(xEnd - xStart) / 6.0) {
    tlog(39, "  Blob invalid");
    band->invalid = true;
    return;
  }
  ...
  whiteCounter = 0;
  otherCounter = 0;
  tlog(39, "scanning from x = %i to %i, y = %i to max(%i, %i)", xStart, xEnd, y, 0, yWhiteStart);
  ...
  if (whiteCounter < 3 || 6 * whiteCounter < 2 * otherCounter) {
    tlog(39,  "Robot: Blob at (%i, %i) thrown out because of too little white above (%i of %i).", band->avgX, band->avgY, whiteCounter, whiteCounter + otherCounter);
    //tlog(39,  "    (%i, %i, %i).", yWhiteStart, xStart, xEnd);
    band->invalid = true;
    return;
  }
  ...
  tlog(39, "scanning from x = %i to %i, y = %i to min(%i, %i)", xStart, xEnd, yEnd, iparams_.height, yEnd + 4 * (int)band->avgWidth);
  // move up from start until you reach a substantial amount of white (1/3 of total pixels in a row)
  ...
  whiteCounter = 0;
  otherCounter = 0;
  tlog(39, "scanning from x = %i to %i, y = %i to min(%i, %i)", xStart, xEnd, yEnd, iparams_.height, yWhiteStart);
  ...
  if (whiteCounter < 3 || 6 * whiteCounter < 2 * otherCounter) {
    tlog(39,  "Robot: Blob at (%i, %i) thrown out because of too little white below (%i of %i).", band->avgX, band->avgY, whiteCounter, whiteCounter + otherCounter);
    //tlog(39,  "    (%i, %i, %i).", yWhiteStart, xStart, xEnd);
    band->invalid = true;
    return;
  }
...
void BandRobotDetector::sanitizeBandCandidateOnElevation(Blob * band, float y) {
  ...
  tlog(39,  "Robot: Blob at (%i, %i) has z height %f", band->avgX, band->avgY, y);
  if (y > 250 || y < -250) {
    tlog(39,  "Robot: Blob at (%i, %i) thrown out because of bad elevation %f", band->avgX, band->avgY, y);
    band->invalid = true;
    return;
  }
...
void BandRobotDetector::detectBlueRobots() {
  ...
  tlog(30, "Detecting robots from %i blobs", verticalBlob[c_BLUE].size());
  int mergeCount = 100;
  ...
  for (uint32_t i = 0; i < verticalBlob[c_BLUE].size(); i++) {
    ...
    if(mergeIndex[i] == mergeCount) {
      ...
      band->invalid = true;
      tlog(39, "Robot: Band %i at x:(%i,%i) ~ %i, y:%i thrown out because it was merged with blob %i", i, band->xi, band->xf, band->avgX, band->avgY, mergeIndex[i]);
    }
    ...
    for (uint32_t j = 0; j < i; j++){
      ...
      if (dx * dx + dy * dy < 100 * 100)
        {
          band->invalid = true;
          tlog(39,"Robot: Blob at (%i, %i) thrown out - too close to blob at (%i, %i)",band->avgX,band->avgY,verticalBlob[c_BLUE][j].avgX,verticalBlob[c_BLUE][j].avgY);
          break;
        }
    }
    if (band->invalid) {
    ...
    tlog(39, "Robot: Band %i at x:(%i,%i) ~ %i, y:%i selected for band check", i, band->xi, band->xf, band->avgX, band->avgY);
    ...
    if (distance < 100 || distance > 6000) { // distance invalid
      tlog(39, "blob thrown out because of distance %5.3f", distance);
      continue;
    }
...
void BandRobotDetector::detectPinkRobots() {
  ...
  std::vector<BlobCollection> verticalBlob = blob_detector_->verticalBlob;
  tlog(30, "Detecting robots from %i blobs", verticalBlob[c_PINK].size());
  int mergeCount = 100;
  uint16_t mergeIndex[mergeCount];
  ...
  for (uint32_t i = 0; i < verticalBlob[c_PINK].size(); i++) {
    ...
    for (uint32_t j = 0; j < i; j++){
      ...
      if (dx * dx + dy * dy < 100 * 100)
        {
          band->invalid = true;
          //tlog(39,"Robot: Blob at (%i, %i) thrown out - too close to blob at (%i, %i)",band->avgX,band->avgY,verticalBlob[c_PINK][j].avgX,verticalBlob[c_PINK][j].avgY);
          break;
        }
    }
    if (band->invalid)
...
void BandRobotDetector::detectRobotCluster() {
  tlog(40,"Detection robot clusters");
  ...
  for (int col = 0; col < NUM_COLS; col++) {
    ...
    tlog(41,"  col %i, white: %f",col,whiteFrac);
    for (int row = 0; row < NUM_ROWS; row++) {
      greenFracs[row][col] = greenCounts[row][col] / CELL_SIZE;
      tlog(41,"    section %i,%i, green: %f",col,row,greenFracs[row][col]);
    }
  ...
  tlog(41,"Best cluster: start: %i length: %i",clusterStart,clusterLength);
  if (clusterLength < MIN_CLUSTER_LENGTH) {
    tlog(40,"Cluster too short: %i",clusterLength);
    return;
  }
  ...
  if (greenCount < MIN_GREEN_COUNT) {
    tlog(40,"Not enough green for cluster: %i",greenCount);
    return;
  }
  ...
  tlog(40,"Raw cluster: %i %f %f",clusterStart,clusterCenter, bear);
  ...
  if (fabs(wo.visionBearing) > DEG_T_RAD * 20) {
    tlog(40,"Cluster thrown out for too high of bearing: %f",wo.visionBearing);
    return;
  }
  wo.seen = true;
  wo.fromTopCamera = (camera_ == Camera::TOP);
  tlog(40,"Cluster found with bearing: %f",wo.visionBearing);
}
