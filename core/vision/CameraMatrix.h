...
class CameraMatrix {
  ...
  public:
    ...
    float getExpectedCameraWidth(int imageX, int imageY, float worldHeight, float worldWidth) const;

    void setCalibration(const RobotCalibration& cal);

    void writeCameraMatrix(char* logname, int counter, bool isBottom) const;
};
