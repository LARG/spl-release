// #include <vision/BallDetector_deepnet.h>
...
// BallDetector_deepnet::BallDetector_deepnet(DETECTOR_DECLARE_ARGS, const ROIDetector& roi_detector, ColorSegmenter& segmenter, BlobDetector& blob_detector) : DETECTOR_INITIALIZE, roi_detector_(roi_detector), color_segmenter_(segmenter), blob_detector_(blob_detector) {
//   // estimator_.setMean(
...
// BallDetector_deepnet::~BallDetector_deepnet() {
// }

// void BallDetector_deepnet::init(TextLogger* tl) {
...
//   classifier_ = std::make_unique<DeepClassifier>("ball_net4_80000", path);
//   classifier_->load();
//   printf("done!\n");
// }

// void BallDetector_deepnet::findBall(std::vector<ROI>& rois) {
...
//     // apply Deep Net to ROI
//     VisionTimer::Start(70, "BallDetector(%s)::classify", camera_);
//     float confidence;
//     // bool result = false;
//     bool result = classifier_->classify(mat, confidence);
//     VisionTimer::Stop("BallDetector(%s)::classify", camera_);
...
//     BallCandidate c;
//     c.centerX = (roi.xmax + roi.xmin)/2;
//     c.centerY = (roi.ymax + roi.ymin)/2;
...
// /*
// bool BallDetector_deepnet::findMovingBall() {
...
// bool BallDetector_deepnet::checkSurroundedByGreen(BallCandidate& candidate){
...
// float BallDetector_deepnet::greenInBox(int xmin, int xmax, int ymin, int ymax){
//   int hstep = 1 << iparams_.defaultHorizontalStepScale, vstep = 1 << iparams_.defaultVerticalStepScale;
...
// bool BallDetector_deepnet::intersectsShoulder(BallCandidate* candidate) {
...
// BallCandidate* BallDetector_deepnet::evaluateMovingBallCandidates(std::vector<BallCandidate>& candidates) {
...
// void BallDetector_deepnet::checkColorsInCircleFit(BallCandidate* candidate, float &whitePct, float &undefPct, float &pctBall){
...
// void BallDetector_deepnet::getBlobContour(Blob* blob, uint16_t *xinit, uint16_t *xfinal) {
...
// void BallDetector_deepnet::fitCircleToPoints(
...
// BallCandidate BallDetector_deepnet::formBallCandidateFromBlob(Blob* blob){
//   int hscale, vscale;
//   color_segmenter_.getStepScale(hscale,vscale);
...
// float BallDetector_deepnet::checkBallColor(BallCandidate &candidate) {
//   int hstep = 1 << iparams_.defaultHorizontalStepScale, vstep = 1 << iparams_.defaultVerticalStepScale;
...
// /*
// float BallDetector_deepnet::checkBelowGreenPct(BallCandidate &candidate, bool useWhite) {
//   // We use the default step sizes because the area under the ball shouldn't be high-res scanned
//   int hstep = 1 << iparams_.defaultHorizontalStepScale, vstep = 1 << iparams_.defaultVerticalStepScale;
...
// bool BallDetector_deepnet::shouldMergeBlobs(BallBlob& b1, BallBlob& b2) {
...
// BlobTriangle BallDetector_deepnet::blobTests(cv::Mat& mat, const ROI& roi, int xstep, int ystep, bool highres) {  
...
// BlobTriangle BallDetector_deepnet::blobGeometryTests(std::vector<BlobTriangle>& triangles, const std::vector<BallBlob>& blobs, const ROI& roi, int xstep, int ystep) {
//   BlobTriangle bestT;
...
// BlobTriangle BallDetector_deepnet::blobGeometryTests(BlobTriangle& t, const std::vector<BallBlob>& blobs, const ROI& roi, int xstep, int ystep) {
...
// float BallDetector_deepnet::max(float a, float b, float c) {
//   return std::max(std::max(a, b), c);
// }

// float BallDetector_deepnet::min(float a, float b, float c) {
//   return std::min(std::min(a, b), c);
// }

// // This function is no longer used. It's been replaced by the macro DOES_CHANGE_COLOR
// // bool BallDetector_deepnet::doesChangeColor(cv::Mat& mat, int row, int col, int window, int& left, int& right) {
// //   left = 0;
...
// ScanLine* BallDetector_deepnet::getRoot(ScanLine& sl) {
//   ScanLine* root = &sl;
...
// void BallDetector_deepnet::merge(ScanLine& top, ScanLine& bot) {
...
// int BallDetector_deepnet::greenPixelCount(const BallBlob& blob, const ROI& roi, int xstep, int ystep) {
...
// std::vector<BallBlob> BallDetector_deepnet::filterBlobs(std::vector<BallBlob> blobs, std::vector<BallBlob>& badBlobs, const ROI& roi, int xstep, int ystep, bool highres) {
...
// bool BallDetector_deepnet::blobTooBig(BallBlob& blob, const ROI& roi) {
...
// bool BallDetector_deepnet::blobTooSmall(BallBlob& blob, const ROI& roi) {
//   int x = blob.centroidX*roi.xstep + roi.xmin;
//   int y = blob.centroidY*roi.ystep + roi.ymin;
//   float expectedBlobSize = cmatrix_.getExpectedCameraWidth(x, y, 80.0, 30.0);
//   if (camera_ == Camera::BOTTOM) {
...
// std::vector<BallBlob> BallDetector_deepnet::computeBlobs(cv::Mat& mat, std::vector<std::vector<ScanLine>>& lines) {
//   int window = 1;
...
// */
// /*
// void BallDetector_deepnet::refineBallLocation(BallCandidate& candidate, const ROI* roi, cv::Mat& mat, BlobTriangle triangle) {
...
// void BallDetector_deepnet::eraseBlob(cv::Mat& mat, BallBlob blob) {
//   for (auto& sl : blob.getScanLineVector()) {
...
// void BallDetector_deepnet::colorBlob(cv::Mat& seg, BallBlob blob, cv::Vec3b color) {
//   for (auto& sl : blob.getScanLineVector()) {
//     for(int x = sl.start; x <= sl.end; ++x) {
...
// void BallDetector_deepnet::createBlobImage(
...
// }
// */
// bool BallDetector_deepnet::setBest(BallCandidate& candidate) {
//   candidate.best = true;
...
// /*
// float BallDetector_deepnet::getExpectedBlobCameraWidth(BallBlob& blob) {
...
// float BallDetector_deepnet::getDirectDistanceByBlobWidth(float dx, int /*imageX*/, int /*imageY*/) {
...
// float BallDetector_deepnet::getDirectDistanceByKinematics(int x, int y) {
//   Position p = cmatrix_.getWorldPosition(x, y, BALL_RADIUS);
//   float dist = cmatrix_.directDistance(p);
//   return dist;
// }

// float BallDetector_deepnet::getDirectDistance(BallCandidate& candidate) {
//   float dist;
