...
float CameraMatrix::getExpectedCameraWidth(int imageX, int imageY, float worldHeight, float worldWidth) const {
  Position pos = getWorldPosition(imageX, imageY, worldHeight);
  float dist = std::sqrt(pos.x*pos.x + pos.y*pos.y + pos.z*pos.z);
  return getCameraWidthByDistance(dist, worldWidth);
}
...
void CameraMatrix::writeCameraMatrix(char* logname, int counter, bool isBottom) const {
  char buff[100];
  if (isBottom) {
    snprintf(buff, sizeof(buff), "/home/jkelle/nao/trunk/BlobImages/%s/bottom/camera_%03i.txt", logname, counter);
  } else {
    snprintf(buff, sizeof(buff), "/home/jkelle/nao/trunk/BlobImages/%s/top/camera_%03i.txt", logname, counter);
  }
