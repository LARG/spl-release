...
LineDetector::LineDetector(DETECTOR_DECLARE_ARGS, ColorSegmenter& segmenter, BlobDetector& blob_detector) :
  ...
  horizontalPoint = color_segmenter_.horizontalPoint;
  verticalPoint = color_segmenter_.verticalPoint;
  horizontalPointCount = color_segmenter_.horizontalPointCount;
  verticalPointCount = color_segmenter_.verticalPointCount;
}

void LineDetector::detectLines(HorizonLine horizon) {
  setHorizon(horizon);
  FormLines(c_WHITE);
  mergeLines();
  generateTransformedImage();       // For drawing
  differentiateCurves();            // Center circle
  FilterLines();
  FormCorners();
  DecodeCorners();
  // Sanmit: These 3 used to happen after goal detection, since it would identify the goal line based on the presence of the goal. Seems like that check was removed when blue goals were removed.
  DetermineEndPoints();
  IdentifyLines();
  DecodeLines();
}
...
void LineDetector::FindHorzLines(FieldLine** currentLine, unsigned char color) {
  ...
  for (uint16_t i = 0; i < verticalBlob[color].size(); i++) {
    if (currentLineCounter == MAX_FIELDLINES) {
      tlog(45, "Horizontal lines at max count of %i", MAX_FIELDLINES);
      break;
    }
...
void LineDetector::FindVertLines(FieldLine** currentLine, unsigned char color) {
  ...
  for (uint16_t i = 0; i < horizontalBlob[color].size(); i++) {
    if (currentLineCounter == MAX_FIELDLINES) {
      tlog(45, "Vertical lines at max count of %i", MAX_FIELDLINES);
      break;
    }
...
/*
      if (color == c_WHITE){ 
        Blob *top = &horizontalBlob[color][retVal.lbIndex[0]];
        Blob *bottom = &horizontalBlob[color][retVal.lbIndex[retVal.lbCount - 1]];
        Position topLeft = cmatrix_.getWorldPosition(top->xi - (top->widthStart / 2), top->yi);
        Position topRight = cmatrix_.getWorldPosition(top->xi + (top->widthStart / 2), top->yi);
        Position bottomLeft = cmatrix_.getWorldPosition(bottom->xf - (bottom->widthEnd / 2), bottom->yf);
        Position bottomRight = cmatrix_.getWorldPosition(bottom->xf + (bottom->widthEnd / 2), bottom->yf);
        float topWidth = sqrt(pow(topLeft.x - topRight.x, 2) + pow(topLeft.y - topRight.y, 2));
        float bottomWidth = sqrt(pow(bottomLeft.x - bottomRight.x, 2) + pow(bottomLeft.y - bottomRight.y, 2));

        tlog(45, "===== LINE topWidth %f bottomWidth %f (%d,%d,%d) (%d,%d,%d)", topWidth, bottomWidth, top->xi, top->yi, top->widthStart, bottom->xf, bottom->yf, bottom->widthEnd);

        if (abs(topWidth - bottomWidth) > 30){
          continue;
        }
      }
*/
...
LineStackRet LineDetector::recurseCheckHorizontal (uint16_t startIndex, LineStackInfo lineStack, unsigned char color) {
  ...
  for (uint16_t j = startIndex + 1; j < verticalBlob[color].size(); j++) {
    ...
    tlog(35,  "%i -> %i: (%i, %i, %i) for (%i, %f, %f)", startIndex, j, posOk, angleOk, widthOk, yDiff, angleDiff, widthFactor);

    if (widthOk && angleOk && posOk) {

      tlog(35,  "%i -> %i", startIndex, j);
      ...
      if (retVal.isLine) {
        if (retVal.lbCount < MAX_BLOBS_PER_LINE) {
          ...
          tlog(45, "This one is pretty horrible.... MAX_BLOBS_PER_LINE exceeded");
        }
  ...
  tlog(35, "Returning from %i, isLine: %i, numPoints: %i, numBlobs: %i, A: %i, B: %i, Threshold: %i", startIndex, bestRetVal.isLine, bestRetVal.pointCount, bestRetVal.lbCount, vparams_.horzValA[color], vparams_.horzValB[color], vparams_.horzValThreshold[color]);
  return bestRetVal;
}
...
void LineDetector::FormLines(unsigned char color) {
  ...
  tlog(30, "After forming, num valid lines are %i", TotalValidLines);
...
void LineDetector::RemoveLinesFromBody() {
  ...
  tlog(30, "After remove lines from body, num valid lines are %i", TotalValidLines);
}
...
void LineDetector::differentiateCurves() {
  ...
  for (int line = 0; line < FieldLinesCounter; line++) {
    ...
    stride = fieldLines[line]->Points / 2 - 1;   // This is just the midpoint index. A circle needs 3 points
    ...
    if (dist < 200) {
      tlog(32, "threw out line for being too short. distance: %5.3f", dist);
      fieldLines[line]->ValidLine = false;
      TotalValidLines--;
      continue;
    }
    ...
    tlog(32, "line dist %5.3f, avg rate slope %5.3f", dist, avgRateSlope);
    ...
    // Todd: stricter curve check on short lines
      ...
      tlog(32, "Line %i, bad slope %f for short line %f", line, avgRateSlope, dist);
      fieldLines[line]->isCurve = true;
      TotalValidLines--;
    }
  ...
  tlog(30, "After diff curves, num valid lines are %i", TotalValidLines);
}
...
bool LineDetector::formCircle(float x1, float y1, float x2, float y2, float x3, float y3, FieldLine* Line, float *cx, float *cy, float *radius, float dist){
  ...
  if (fabs(y1-y3) < 200) {
    tlog(32,  "points too close side2side %5.5f", (y3-y1));
    return false;
  }

  // distance in x (forward)
  if ((maxx-minx) < 120) {
    tlog(32,  "points too close in xdist %5.5f", (maxx-minx));
    return false;
  }

  // and full distance
  if (dist < 300){
    tlog(32,  "points too close in fulldist %5.5f", dist);
    return false;
  }

  tlog(32,  "Circle xdist %5.5f, ydist %5.5f, fulldist %5.5f", (maxx-minx), (y3-y1), dist);

  if (maxx > 4000) {
    tlog(32,  "points too far away %5.5f", maxx);
    return false;
  }
  ...
  *cy = circleCenter.y;
  ...
  tlog(32,  "circle radius: %5.5f", *radius);

  tlog(32,  "circle: (%5.5f, %5.5f)", circleCenter.x, circleCenter.y);

  if (fabs(*radius - CIRCLE_RADIUS) > 180){
    tlog(32,  "circle radius wrong: %f, exp: %f", *radius, CIRCLE_RADIUS);
    return false;
  }

  // how many points
  tlog(32, "Circle numPoints %i", Line->Points);
  ...
  for (int i = 0; i < numPoints; i++){
    ...
    tlog(32, "Sampled pt %i is %5.5f from circle radius: %5.5f", i, dist, *radius);

    if (fabs(dist - *radius) > 40) {
      tlog(32, "  distance too high, throwing circle out");
      return false;
    }
    ...
    if (i > 0){
      ...
      tlog(32, "  Angle from previous sample point: %5.5f", angle);

      // compare with last
      if (i > 1){
        float change = fabs(angle-lastAngle);
        tlog(32, "  Slope change: %f", change);
        float slopeChangeLimit = 0.7;//M_PI / 8;
        if (change > slopeChangeLimit && change < 2 * M_PI - slopeChangeLimit){
          tlog(32, "Slope change is too high %f > %f", change, slopeChangeLimit);
          return false;
        }
      }
      lastAngle = angle;
...
bool LineDetector::formPenaltyCross(float x1, float y1, float x2, float y2, int line) {
  ...
  tlog(32,  "line %i penalty cross bounding box (%i, %i) to (%i, %i)", line, left, top, right, bottom);
  ...
  tlog(32, "line %i penalty cross green %i vs undefinedCounter %i in total %i", line, greenCounter, undefinedCounter, totalCounter);

  if (greenCounter > 4 * undefinedCounter) {
    tlog(32, "Line %i detected as penalty cross", line);
...
void LineDetector::mergeLines() {
  for (int line1 = 0; line1 < FieldLinesCounter; line1++) {
    ...
    for(int line2 = line1+1; line2 < FieldLinesCounter; line2++) {
      if(FieldLinesCounter >= MAX_FIELDLINES) return;
  ...
  tlog(30, "After merge lines, num valid lines are %i", TotalValidLines);
}
...
bool LineDetector::mergeFieldLines(FieldLine * line1, FieldLine * line2) {
  ...
  if (angleDiff < 7 || angleDiff > 180 - 7) {
    ...
    if ((offsetYDiff < 10 && line1->Offset < 2 * iparams_.height && line1->Offset > - iparams_.height) ||
      ...
      tlog(38, "horizontal: %i, %i vs %i, with slope %f", horizontalCheck, counter1Start, counter2Start, line1->Slope);
      ...
      CalLineDetails(fieldLines[FieldLinesCounter]); 
...
void LineDetector::FilterLines() {
  ...
  tlog(30, "After filter lines, num valid lines are %i", TotalValidLines);
}

bool LineDetector::FilterLine(FieldLine* line){

  // Filter line based on length in the image. I.e. removes lines that are too short.

  if (line->length < 110) {
    line->ValidLine = false;
    TotalValidLines--;
    tlog(35, "         line thrown on length < 110");
    return false;
  }
  ...
  // Filter based on thickness of lines on the ground
  if (dist > 160 || distanceSqr < 16) {
    line->ValidLine = false;
    TotalValidLines--;
    tlog(35, "        line thrown on dist>160 || distanceSqr<16");
    return false;
  }
  ...
  Position relPos = cmatrix_.getWorldPosition(line->bR.x, line->bR.y);


  // Additional filters for vertical lines to avoid finding robots?
  // Basically, if a line appears vertical, it should stretch all the way towards us (except the one by the penalty box). 

/*  
  if (line->isVertical && abs(line->Slope) > 10 && sqrtf(pow(relPos.x, 2) + pow(relPos.y, 2)) > 1000){
    line->ValidLine = false;
    TotalValidLines--;
    tlog(35, "         line thrown for being vertical but too far away");
    return false;
  }
*/
  tlog(35, "         line validated: thickness %f, length %d, vertical %d, bottom (%f,%f) Slope %f rateSlope %f relPos (%f,%f)", dist, line->length, line->isVertical, line->bR.x, line->bR.y, line->Slope, line->rateSlope, relPos.x, relPos.y);
...
void LineDetector::IdentifyLines() {
  ...
  WorldObject *wo = vblocks_.world_object->objects_.data();
  ...
  for (int i = 0; i < FieldLinesCounter; i++) {
    ...
    if (!line->ValidLine || line->isCurve || line->onCircle || line->isCross) {
      tlog(38, "    Line %i is not a line",i);
      continue;
    }
    ...
    if ((!goalLineSeen && goalSeen)|| (!centerLineSeen && centerCircleSeen)) {
      ...
      LineSegment lineEq(line->pt1, line->pt2);
      ...
      if (distance < 150) {
        ...
        tlog(38, "    Line %i seen attached to object. Add to object %i of (BGL = %i, YGL = %i, CL = %i)", i, finalLineIndex, WO_OPP_GOAL_LINE, WO_OWN_GOAL_LINE, WO_CENTER_LINE);
        continue;
      }
  ...
  for (int i = 0; i < FieldLinesCounter; i++) {
    ...
    tlog(38, "    Line %i angle %f vs %f. Dist %f", i, lineAngle, goalLineAngle, distance);

    // Try parallel penalty box line
    if (angleDiff < 10 || angleDiff > 180 - 10) {
      if (distance < 700 && distance > 400) {
        line->identifiedIndex = finalLineIndex;
        tlog(38, "    Line %i is the penalty line", i);
        break;
      }
    }
    ...
    /*
      ...
      tlog(38, "    Line %i is the penalty side line", i);
      }
      }
    */
  }
...
void LineDetector::DecodeLines() {
  ...
  if (nextLine > lastLine){
    tlog(39, "    No Lines Slots left to be filled in)");
    return;
  }
  ...
  for (int i = 0; i < FieldLinesCounter; i++){
    ...
    if (line->identifiedIndex == -1 && nextLine > lastLine) {
      tlog(39,"    Ran out of WOs to fill. %i / %i lines remaining", TotalValidLines, FieldLinesCounter);
      continue;
    }
    ...
    if (line->identifiedIndex == -1) {
      tlog(39,"    Unidentified Line %i seen, added to worldObject %i", i, nextLine);
      wo= &(vblocks_.world_object->objects_[nextLine]);
      nextLine++;
    } else {
      tlog(39,"    Indentified Line %i seen, added to worldObject %i", i, line->identifiedIndex);
...
void LineDetector::FormCorners() {
  ...
  if (TotalValidLines < 2) {
    tlog(36,  "    Too few lines found (%i).", TotalValidLines);
    return;
  }
  ...
  for (lineIDStart = 0; lineIDStart < FieldLinesCounter - 1; lineIDStart++) {
    ...
    if (!fieldLines[lineIDStart]->ValidLine || fieldLines[lineIDStart]->isCurve ||
      ...
      tlog(36, "  Line %i is not a valid line", lineIDStart);
      continue;
    }
    ...
    for (int lineIDCheck = lineIDStart + 1; lineIDCheck < FieldLinesCounter; lineIDCheck++) {
      ...
      if (angleOK && xOK && yOK && horizonOK) {
        ...
        if (CornerPointCounter >= MAX_CORNERPOINTS) {
          tlog(36, "    Ran out of cornerPoints to fill. Increase MAX_CORNERPOINTS!!!");
          return;
        }
        ...
        // FYI: I don't know what this is
        cornerPoints[CornerPointCounter]->Orientation = 1;

        tlog(36, "    Detected corner between %i and %i of type %i (1 = T, 0 = L)", lineIDStart, lineIDCheck, cornerPoints[CornerPointCounter]->CornerType);

        CornerPointCounter++;

      } else {
        tlog(36, "  Corner between %i and %i failed check (ang = %i, pos = %i, horizon = %i)", lineIDStart, lineIDCheck, angleOK, xOK && yOK, horizonOK);
      }
    }
  }
}
...
void LineDetector::DecodeCorners() {
  ...
  tlog(37, "DecodeCorners()");

  if (CornerPointCounter > 4 || CornerPointCounter < 1) {
    tlog(37, "    Too many or zero corner points found (%i). Not Setting WOs", CornerPointCounter);
    return;
  }
  ...
  for (x = 0; x < CornerPointCounter; x++) {
    ...
    if (cornerPoints[x]->CornerType == 0) {
      if (lCount < 2) {
        ...
        lCount++;
        tlog(37, "  Filling L corner %i in WO %i", x, tempLocation);
      } else {
        tlog(37, "  Ran out of L corners to fill");
        continue;
      }
      ...
      if (tCount < 2) {
        ...
        tCount++;
        tlog(37, "  Filling T corner %i in WO %i", x, tempLocation);
      } else {
        tlog(37, "  Ran out of T corners to fill");
        continue;
      }
    }
