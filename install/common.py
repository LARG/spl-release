...
def runCommand(command,password=None,output=True,exit_on_error=False):
  ...
  outstr = ""
  while True:
    i = p.expect(['Password:',r'\[sudo\] password.*', r'Are you sure.*\(yes/no\)? *', r'.*\[Y\/n\]','.+\n', '.+', pexpect.EOF])
    if i == 6: break
    ...
    if i == 2:
      ...
      outstr = outstr.strip()
      outstr = re.sub(r'\s*Connection to (\d+\.){3}\d+ closed\.\s*', '', outstr)
      outstr = re.sub(r'\s*Password:\s*', '', outstr)
      if output: print outstr
    elif i == 5:
      outstr = outstr.strip()
      if output:
...
def runLocalCommand(command,password=None,output=None,exit_on_error=False):
  ...
  return runCommand(command,password,output=output,exit_on_error=exit_on_error)
...
def runRemoteCommand(ip,command,root=False,output=None,exit_on_error=False):
  ...
  if root:
    ...
    return runCommand(command,password='root',output=output,exit_on_error=exit_on_error)
  else:
...
def copyFiles(ip,files,dest='',root=False):
  if root:
    ...
    runRemoteCommand(ip, "mv %s %s" % (directory + "/" + base,dest),root=True)
  else:
