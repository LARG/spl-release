...
naointerface::naointerface(boost::asio::io_service& io_service) : frame_handler(io_service)
{
  ...
  initMemory();
  audio_wrapper_ = new AudioWrapper();
  audio_wrapper_->init(audio_block_); 
  // // set up locks
...
void naointerface::postProcess() {
  ...
  // std::cout << "Incremented frame " << frame_info_->frame_id << std::endl;
  
  populateSensors();

  // TODO: reenable? Not sure... - Josiah
  // if (USE_AL_MOTION)
...
void naointerface::preProcess() {
  ...
  std::vector<float> joint_angle_positions(NUM_JOINTS);
  std::vector<float> joint_stiffnesses(NUM_JOINTS);
  for(int i = 0; i < joint_angle_positions.size(); ++i) {
    // if (i == LAnklePitch)
    joint_angle_positions[i] = raw_joint_commands_->angles_[i];
    joint_stiffnesses[i] = raw_joint_commands_->stiffness_[i];
  }
  frame_handler.set_actuator_values(raw_joint_commands_, raw_joint_angles_, led_commands_);

  if (USE_AL_MOTION) {
    // TODO: replace with LOLA
...
void naointerface::populateSensors(){
  // Read sensors from socket.
  // int offset = 0;
  // Get positions

  frame_handler.get_sensor_values(raw_joint_angles_, raw_sensors_);
